...
namespace XSDFrontend
{
	namespace Attribute
	{
		AttributeGroup::AttributeGroup(void)
			...
			m_attributes(), m_attributeGroups(), m_anyAttribute(nullptr)
		{
		}
		...
		AttributeGroup::AttributeGroup(const AttributeGroup & ano)
			...
			m_attributes(ano.m_attributes), m_attributeGroups(ano.m_attributeGroups), m_anyAttribute(new AnyAttribute(*ano.m_anyAttribute))
		{
		}

		AttributeGroup::AttributeGroup(const AttributeGroup && ano)
			: IXSDNamedElementInterface(std::move(ano)), IXSDReferenceElementInterface(std::move(ano)),
			m_attributes(std::move(ano.m_attributes)), m_attributeGroups(std::move(ano.m_attributeGroups)), m_anyAttribute(new AnyAttribute(*ano.m_anyAttribute))
		{
		}
