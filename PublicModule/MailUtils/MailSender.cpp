...
namespace SSUtils
{
	namespace Mail
	{
		MailSender::MailSender(const std::string & _senderMail, const std::string & _senderPassword, const std::string & _senderServer, const std::string _receiverMail)
			: senderMail(_senderMail), senderPassword(_senderPassword), senderServer(_senderServer), receiverMails(1, _receiverMail)
		{
		}

		MailSender::MailSender(const std::string & _senderMail, const std::string & _senderPassword, const std::string & _senderServer, const std::vector<std::string>& _receiverMails)
			: senderMail(_senderMail), senderPassword(_senderPassword), senderServer(_senderServer), receiverMails(_receiverMails)
		{
		}

		std::pair<bool, std::string> MailSender::operator()(const MessageType type, const std::string & title, const std::string & content, const CharType charType)
		{
