...
namespace XSDAnalyzer
{
	...
	std::shared_ptr<XSDFrontend::Attribute::Attribute> AttributeAnalyzer::loadAttribute(const std::shared_ptr<SSUtils::XML::Node> node)
	{
		...
		{
			attribute->setForm(XSDFrontend::Attribute::Attribute::String2Form.left.find(node->getAttr(XSDFrontend::Token::FormAttr()))->second);
		}
		...
		for (const auto &child : node->getChildren())
		{
			if (child != nullptr)
			{
				if (child->getTag() == XSDFrontend::Token::AnyAttributeTag())
				{
					...
					{
						anyAttribute->setProcessContents(XSDFrontend::Attribute::AnyAttribute::String2ProcessContents.left.find(child->getAttr(XSDFrontend::Token::ProcessContentsAttr()))->second);
					}
